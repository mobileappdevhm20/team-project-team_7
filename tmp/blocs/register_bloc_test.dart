...
void main() {
  ...
  group('Test email and passowrd changed validators', () {
    ...
    blocTest(
      ...
      expect: [
        isA<RegisterState>().having((source) => source.isPasswordValid,
    ...
    blocTest(
      ...
      expect: [
        isA<RegisterState>().having(
            (source) => source.isNameValid, "name should not be valid", false)
      ],
    );
    ...
    blocTest(
      ...
      expect: [
        isA<RegisterState>().having(
            (source) => source.isNameValid, "name should be valid", true)
      ],
    );
  });
  ...
  group('Test Register', () {
    ...
    blocTest(
      'Login failure',
      build: () async => registerBloc,
      act: (bloc) async => bloc.add(const Submitted(
          email: "test@hm.error", password: "error", name: "Testname")),
      expect: [
        isA<RegisterState>()
    ...
    blocTest(
      ...
      build: () async => registerBloc,
      act: (bloc) async => bloc.add(const Submitted(
          email: "test@hm.edu", password: "Test1234", name: "Testname")),
      expect: [
        isA<RegisterState>()
            .having((source) => source.isSubmitting, "submitting", true),
        isA<RegisterState>()
